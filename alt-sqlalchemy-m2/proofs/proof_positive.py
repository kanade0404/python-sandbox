"""POSITIVE evidence against the GENERATED facade (M2).

This is M1's `proof_positive.py`, re-pointed at `generated.pg.facade` -- the
module `altsa_gen` produced from live PostgreSQL 16 -- and widened to the EC
schema. Claims C1..C8 are unchanged in substance; C9..C11 are new surface that
M2 added (aggregates, typed writers, declared-shape strategy).
...
def c2_arithmetic() -> None:
    ...
    # The repo_pg.py hole this closes: SQLAlchemy types
    # `OrderItems.quantity * OrderItems.unit_price` as ColumnElement[int] and
    # `unit_price * quantity` as ColumnElement[Decimal] -- same SQL, different
    # static type. Here BOTH orders are impossible to get wrong, because
    # arithmetic exists only as a method on the left-hand NumCol.
    reveal_type(ORDER_ITEMS.unit_price.mul(ORDER_ITEMS.quantity))  # Expr[Decimal]
    reveal_type(ORDER_ITEMS.quantity.mul(3))  # Expr[int]
...
def c3_join_nullability() -> None:
    base = from_orders()
    reveal_type(base)  # QOrders     -- has NO `.users` (see proof_negative N4)
    ...
    reveal_type(mixed)  # QOrdersUsersIOrderItemsN
    reveal_type(mixed.users.email)  # Col[Literal['users'], str]
    reveal_type(mixed.order_items.quantity)  # NumCol[..., int | None]

    # nullability is TRANSITIVE: anything joined after a LEFT JOIN is nullable
    # too, because the whole right-hand branch may be absent.
    chain = from_orders().left_join_order_items().left_join_products()
    reveal_type(chain)  # QOrdersOrderItemsNProductsN
    reveal_type(chain.products.sku)  # Col[Literal['products'], str | None]
...
# ===========================================================================
# C9 (new in M2) -- aggregates are HONEST about the empty group
# ===========================================================================


def c9_aggregates() -> None:
    q = (
        from_users()
        .join_orders()
        .join_order_items()
    ...
    reveal_type(q)  # QUsersOrdersIOrderItemsI

    revenue = sum_(ORDER_ITEMS.unit_price.mul(ORDER_ITEMS.quantity))
    reveal_type(revenue)  # Expr[Decimal | None]

    # SUM over an empty group is NULL, and the type says so. SQLAlchemy's
    # `func.sum(...)` claims the bare element type instead.
    sel = q.select(USERS.email, revenue)
    reveal_type(sel)  # Select[str, Decimal | None]

    # `coalesce` is how you get a total value back, with the type to match
    total = coalesce(revenue, Decimal("0"))
    reveal_type(total)  # Expr[Decimal]
    reveal_type(q.select(USERS.email, total))  # Select[str, Decimal]

    reveal_type(q.select(USERS.email, count_star()))  # Select[str, int]


# ===========================================================================
# C10 (new in M2) -- typed writers: types AND completeness
# ===========================================================================


def c10_writers() -> None:
    conn = Conn("sqlite://")

    # `user_id` is NOT NULL with no server default -> a REQUIRED keyword.
    # `id`, `status`, `version`, `total`, `created_at` all have server
    # defaults -> omissible. Getting that from the schema is exactly what a
    # hand-written model cannot do.
    reveal_type(insert_orders(conn, user_id=uuid4()))  # int
    reveal_type(
        insert_orders(
            conn,
            id=uuid4(),
    ...
    # UPDATE assignments are TABLE-TAGGED, and the right-hand side may be an
    # expression over the same column (optimistic locking, in one statement).
    reveal_type(
        update_orders(
            conn,
            ORDERS.status.set(OrderStatus.PAID),
...
# ===========================================================================
# C11 (new in M2) -- the shape strategy, positively
# ===========================================================================


def c11_declared_shapes() -> None:
    # single-edge shapes are AUTO-generated from the FK graph, in both
    # directions and both join kinds -- nothing was declared for these
    reveal_type(from_orders().left_join_payments())  # QOrdersPaymentsN
    reveal_type(from_payments().join_orders())  # QPaymentsOrdersI
    reveal_type(from_products().join_order_items())  # QProductsOrderItemsI
    reveal_type(from_staff_accounts().join_accounts())  # QStaffAccountsAccountsI

    # a DECLARED 2-edge shape exists ...
    reveal_type(from_orders().left_join_users().left_join_order_items())
    # ... and so does the mixed-kind one that was declared separately
    reveal_type(from_orders().join_users().left_join_order_items())
    # The inner/inner combination was NOT declared: see proof_negative N24.
...
PUBLIC_MODULES = [
    ...
    "altsa_runtime.query",
    "altsa_runtime.conn",
    "altsa_runtime.catalog",
    "altsa_runtime.ir",
    "altsa_runtime.sentinel",
]
...
def assert_no_sqlalchemy_leakage() -> list[str]:
    ...
    # `_backend` is the designated quarantine and is EXPECTED to hold them.
    runtime = importlib.import_module("altsa_runtime._backend")
    quarantined = sum(
        1
        ...
        and getattr(
            getattr(runtime, a),
            "__module__",
            getattr(getattr(runtime, a), "__name__", ""),
        ).split(".")[0]
        == "sqlalchemy"
    )
    print(
        f"[leakage] public modules {len(PUBLIC_MODULES)}: {len(findings)} "
        f"SQLAlchemy names found (want 0); altsa_runtime._backend quarantine "
        f"holds {quarantined}",
        file=sys.stderr,
    )
    for f in findings:
...
def assert_no_any_in_public_signatures() -> list[str]:
    """Fail if a PUBLIC callable in the generated facade mentions `Any`.

    Overload *implementation* signatures are exempt: pyright never resolves a
    call against them, and `inspect.signature` sees only the last def.
    """
    import inspect

    from generated.pg import facade
