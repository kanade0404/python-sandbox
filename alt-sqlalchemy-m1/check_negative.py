...
Usage:  uv run pyright --project pyrightconfig.negative.json --outputjson \
          | uv run python check_negative.py
"""
...
SRC = Path(__file__).parent / "src" / "proof_negative.py"
...
def main() -> int:
    ...
    if not missing and not unexpected:
        print(
            ...
            f"({len(errors)} diagnostics total; pyright emits an extra "
            f"reportUnknownMemberType alongside each attribute-access error)"
        )
        return 0
    return 1
